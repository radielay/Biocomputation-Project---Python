    ...
    for i in range(0, P):
        ...
        for j in range(0, N):
            ...
            if mut_prob < mut_rate:       # Mutation function
                alter = random.uniform(-32, mut_step)
                if gene < 32:
                    gene += alter  # Add float from a defined range, if the gene is smaller than 32
                    if gene > 32:
                        gene = 32
                else:
                    gene -= alter  # Subtract float from a defined range, if the gene is bigger than 32
                    if gene < -32:
                        gene = -32
